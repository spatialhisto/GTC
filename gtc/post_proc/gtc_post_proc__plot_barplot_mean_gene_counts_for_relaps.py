...
def plot_barplot_mean_gene_counts_for_relaps(
    main_output_dir: str,
    ...
    mean gene counts in the
    'neoplastic' tissue for relaps
    and non-relaps samples, one
    standard deviation as error bar
    and the count values as points."""

    DEBUG_PLOT_ONLY_1_SERIES = False

    #plot_size = (29.0, 20.0); img_dims_plots = (400, 400); pos_txt_box = (0.50, -0.04); num_rows_per_plot = 10; num_genes_per_row = 10; font_size_box = 9; save_res = 300

    ####

    path_out = main_output_dir + 'Summary_barplot_mean_gene_counts_for_relaps/'
                    ...
                    for num_plot in range(num_plots):
                        ...
                        for count_gene_plot, gene_sub in enumerate(genes_sub):  # loop over every gene
                            ...
                            if gene_sub not in ['empty']:
                                ...
                                def split_gene_counts_relating_to_relaps(occurrence):
                                    counts = [count for (count, sample) in zip(gene_counts, samples)
                                              if dict_disease_history[sample] == occurrence]

                                    data['relaps'] += [occurrence for _ in range(len(counts))]
                                    data['counts'] += list(counts)

                                split_gene_counts_relating_to_relaps('yes')
                                split_gene_counts_relating_to_relaps('no')

                                df_gene_counts = pd.DataFrame(data)

                                if df_gene_counts['counts'].size / 2 > 1:  # enough values for standard deviation
                                    sns.barplot(ax=ax, x='relaps', y='counts', hue='relaps', data=df_gene_counts,
                                ...
                                sns.swarmplot(ax=ax, x='relaps', y='counts', data=df_gene_counts, color='royalblue', size=5)
                        ...
                        plt.suptitle('Relaps occurence', fontsize=20, x=0.5, y=1.0)
